<template>
  <div class = "size-full space-x-hidden">
    <div class = "flex justify-between items-center w-full h-20 bg-gray-100 mt-3 shadow-lg antialiased">
      <h1 class = "flex font-medium font-amaticBold text-2xl ml-5 ">Events Calendar</h1>
      <button class = "flex justify-end items-center lg:mr-8 p-2 text-md font-semibold font-amaticRegular h-10 bg-gray-200 rounded-md shadow-md transform-transition duration-300 transform 
      hover:scale-110 hover:bg-blue-200 "  @click="toggleSidebar">
        Add events</button>
    </div> 
    <aside
  class="fixed top-12 right-0 w-full md:w-[450px] h-full bg-[#f8f9ef] shadow-lg transition-transform z-50 custom-shadow"
  ...
  <div class="p-4">
    ...
    <div class = "flex items-center">
      <label for="event-title" class="block text-md font-semibold text-gray-700 mr-2 font-raleway">Event Name: </label>
       <input type="text" class = "h-8 w-56 rounded-lg shadow-md text-sm"
        id ="eventTitle" v-model="eventTitle" required>
      </div>
    </div>
    <div class = "mt-8 ml-2">
    <div class = "flex items-center">
      <label for="event-title" class="block text-md font-semibold text-gray-700 mr-2 font-raleway">Venue: </label>
       <input type="text" class = "h-8 w-56 rounded-lg shadow-md text-sm"
        id ="eventVenue" v-model="eventVenue" required>
      </div>
    </div>
    <div class = "mt-8 ml-2">
    <div class = "flex items-center">
      <label for="event-title" class="block text-md font-semibold text-gray-700 mr-3 font-raleway">Date: </label>
       <input type="date" class = "h-8 w-56 rounded-lg shadow-md text-sm"
        id ="eventDate" v-model="eventDate" required>
      </div>
    </div>  
    ...
    <div class = "flex items-center">
      <label for="event-title" class="block text-md font-semibold text-gray-700 mr-3 font-raleway">Start Time: </label>
       <input type="time" class = "h-8 w-56 rounded-lg shadow-md text-sm"
        id ="startTime" v-model="eventStartTime" required>
      </div>
    </div>
    <div class = "mt-4 ml-2">
    <div class = "flex items-center">
      <label for="event-title" class="block text-md font-semibold text-gray-700 mr-3 font-raleway">End Time: </label>
       <input type="time" class = "h-8 w-56 rounded-lg shadow-md text-sm"
        id ="endTime" v-model="eventEndTime" required>
      </div>
    </div>    

    <div class = "mt-10">
    <button type="submit" class = "h-10 bg-blue-400 text-md font-bold px-2 rounded-lg shadow-lg">Set Schedule</button>
